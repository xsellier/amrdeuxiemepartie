 ...
 */
void print_matrix(bool** matrix, int size) {
	for (int i = 0; i < size; ++i) {
 ...
 * file flux
 */
void k_color(bool** matrix, int size, int k, ofstream &file) {
 ...
 */
void clique(bool** matrix, int size, ofstream &file) {
 ...
 * graph
 */
void invert_matrix(bool **matrix, int size) {
	for (int i = 0; i < size; ++i)
		for (int j = 0; j < size; ++j)
			if (i != j)
 ...
 */
void independent_set(bool **matrix, int size, ofstream &file) {
	if (size == 2) {
		ostringstream clauses;
		clauses.clear();
		file << "p cnf " << 2 << " " << 3 << endl;
 ...
 */
void vertex_cover(bool **matrix, int size, ofstream &file) {
	invert_matrix(matrix, size);
 ...
 */
void hamitonian_common(ostringstream &result, bool **matrix, int size,
		ofstream &file) {
...
void hamiltonian_path(bool **matrix, int size, ofstream &file) {
...
void hamiltonian_circuit(bool **matrix, int size, ofstream &file) {
 ...
 */
void add_var(string text_var, vector<bool> &var) {
 ...
 * in the vector.
 */
void get_var_result(vector<bool> &var, string result) {
 ...
 */
void execute_minisat(string minisat, string result, vector<bool> &var) {
 ...
 */
void analyse_k_color(vector<bool> &var, int k) {
	int size = var.size();
	...
	for (int i = 0; i < size; i = i + k)
		for (int j = i; j < i + k; ++j)
			if (var[j])
				cout << "le sommet " << (int) (i / k) << " a la couleur " << j
						- i << endl;
 ...
 */
void analyse_clique(vector<bool> &var, int wanted_size) {
	int size = var.size();
	int clique_size = 0;
	...
	if (size == 0)
		...
		if (wanted_size > clique_size) {
			...
			cout << "Voici une clique de taille " << wanted_size << " :"
					<< endl;
 ...
 */
void analyse_independent_set(vector<bool> &var, int wanted_size) {
	int size = var.size();
	int set_size = 0;
 ...
 */
void analyse_hamiltonian_common(vector<bool> &var, int nb_node, int size) {
	int order[nb_node];
 ...
 */
void analyse_hamiltonian_path(vector<bool> &var, int nb_node) {
	int size = var.size();
 ...
 * circuit
 */
void analyse_hamiltonian_circuit(vector<bool> &var, int nb_node) {
	int size = var.size();
 ...
 *  a vertex_cover.
 */
void anlyse_vertex_cover(vector<bool> &var) {
	int size = var.size();
